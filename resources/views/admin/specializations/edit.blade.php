...
@section('title', 'Admin: Edit Specialization')

@section('content')
<div class="container mt-4">
    <div class="card">
        <div class="card-header">
            <h1>Edit Specialization: {{ $specialization->name }}</h1>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('admin.specializations.update', $specialization->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('skill_id') is-invalid @enderror" id="skill_id" name="skill_id" required>
                        ...
                        <option value="{{ $skill->id }}" {{ old('skill_id', $specialization->skill_id) == $skill->id ? 'selected' : '' }}>{{ $skill->name }}</option>
                        @endforeach
                    </select>
                    @error('skill_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code" value="{{ old('code', $specialization->code) }}" required placeholder="e.g., PILOTING_SPACE_ADVANCED">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('xp_cost') is-invalid @enderror" id="xp_cost" name="xp_cost" value="{{ old('xp_cost', $specialization->xp_cost) }}" required>
                     @error('xp_cost')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                 ...
                 <div class="mb-3 form-check">
                    <input type="checkbox" class="form-check-input" id="restricted" name="restricted" {{ old('restricted', $specialization->restricted) ? 'checked' : '' }}>
